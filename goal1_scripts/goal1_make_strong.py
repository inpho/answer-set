...
def print_evaluations(type=IdeaEvaluation):
    ...
    for eval in evals:
        ...
        if eval.relatedness == 0:
            print "p0(i%s, i%s)." % (eval.ante_id, eval.cons_id)
        if eval.relatedness == 1 or eval.relatedness == 2:
            print "p1(i%s, i%s)." % (eval.ante_id, eval.cons_id)
        if eval.relatedness == 3 or eval.relatedness == 4:
            print "p4(i%s, i%s)." % (eval.ante_id, eval.cons_id)
